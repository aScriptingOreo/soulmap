...
export function updateMetaTags(location: Location & { type: string } | null, coords: [number, number]) {
  if (!location) {
    // Update title for coordinates only
    document.title = `Coordinates [${Math.round(coords[0])}, ${Math.round(coords[1])}] - Soulmap`;
    document.querySelector('meta[name="description"]')?.setAttribute('content', 
        `Viewing coordinates [${Math.round(coords[0])}, ${Math.round(coords[1])}] in Soulmap.`);
    return;
  }
...
// Export createTemporaryMarker for use in search.ts
export function createTemporaryMarker(latlng: L.LatLng, map: L.Map): L.Marker {
...
// Add a new function to remove the temporary marker
export function removeTemporaryMarker(map: L.Map): void {
  if (tempMarker) {
      map.removeLayer(tempMarker);
      tempMarker = null;
  }
  ...
  try {
      ...
      map.on('click', (e: L.LeafletMouseEvent) => {
          ...
          if (closestDistance < minDistance) {
              ...
              // Find the nearest named location for relative position
              const nearestNamedLocation = locations
              ...
              // Show coordinate-only view with relative location
              sidebar.updateContent(null, coords[0], coords[1], nearestNamedLocation);
              
              // Update document title for coordinates
              updateMetaTags(null, coords);
          }
      });
      ...
      markerModal.onSubmit = (data) => {
          ...
          // Update sidebar content to show the new marker information
